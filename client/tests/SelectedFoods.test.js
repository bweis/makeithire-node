...
describe('SelectedFoods', () => {
  ...
  describe('when prop `foods` is blank', () => {
    ...
    it('should display a zero kcal value', () => {
      expect(wrapper.find('#total-kcal').first().text()).toEqual('0.00');
    });
  });
  ...
  describe('when `foods` is populated', () => {
    ...
    const totalKcals =
      foods.reduce((memo, f) => parseFloat(f.kcal) + memo, 0.0).toFixed(2);
    ...
    it('should total appropriate kcal', () => {
      expect(wrapper.find('#total-kcal').first().text())
